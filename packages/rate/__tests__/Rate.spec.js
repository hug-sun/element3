...
describe('Rate', () => {
  describe('props', () => {
    ...
    it('colors & icon-classes', async() => {
      ...
      expect(wrapper.find('.icon-rate-face-1').element.style.color).toBe('rgb(153, 169, 191)')

      await wrapper.findAll('.el-rate__item')[2].trigger('click')
      await nextTick()

      expect(wrapper.find('.icon-rate-face-2').exists()).toBeTruthy()
      expect(wrapper.find('.icon-rate-face-2').element.style.color).toBe('rgb(247, 186, 42)')

      await wrapper.findAll('.el-rate__item')[4].trigger('click')
      await nextTick()

      expect(wrapper.find('.icon-rate-face-3').exists()).toBeTruthy()
      expect(wrapper.find('.icon-rate-face-3').element.style.color).toBe('rgb(255, 153, 0)')
    })
    ...
    it('score-template & show-score', () => {
      ...
      expect(console.warn).toBeCalledWith('[Element Migrating][ElRate][Attribute]: text-template is renamed to score-template.')
